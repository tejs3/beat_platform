...
<script setup lang="ts">
  ...
  const setupStepCtx = () => {
    const { id, serviceId, creationMode, type } = route.params as Record<string, string | undefined>
    createStore.setStepContext({
      clusterId: Number(id),
      serviceId: serviceId != null && serviceId !== '' ? Number(serviceId) : 0,
      creationMode: (creationMode as StepContext['creationMode']) || 'internal',
      type: type as StepContext['type']
    })
  }
