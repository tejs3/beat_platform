...
<script setup lang="ts">
  ...
  import { listParcelServices } from '@/api/beat'
  ...
  const targetServiceName = computed(() => {
    const sid = stepContext.value.serviceId
  ...
  const buildCompleteServiceInfo = (
    ...
    const name = targetServiceName.value
    if (!name) {
      throw new Error('Service not found for add-component')
    }
    const inherent = inherentServices.find((v) => v.name === name)!
    const detail = { ...installedServiceDetailMap.get(name)! }
  ...
  const markSelectedAsInstalled = async (hasInstalled: boolean) => {
    ...
    const parcelFiltered = inherentServices.filter((s) =>
      allowed.has(String(s.name || '').toLowerCase())
    )
    ...
    if (type === 'component') {
      ...
      if (!installedServiceDetailMap.has(targetServiceName.value)) {
        message.error(t('common.no_data'))
        return
      }
      const completeService = buildCompleteServiceInfo(installedServiceDetailMap, parcelFiltered)
  ...
  // onActivated alone misses first mount when this step is not keep-alive wrapped
  onMounted(bootstrap)
