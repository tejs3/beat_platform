...
public class ComponentServiceImpl implements ComponentService {
    ...
    public PageVO<ComponentVO> list(ComponentQuery query) {
        ...
        try (Page<?> ignored =
            ...
            componentPOList.forEach(component -> {
                ...
                enrichProcess(componentVO, component);
    ...
    public ComponentVO get(Long id) {
        ...
        enrichProcess(componentVO, componentPO);
        return componentVO;
    }

    private void enrichProcess(ComponentVO componentVO, ComponentPO componentPO) {
        if (componentVO == null || componentPO == null) {
            return;
        }
        Long clusterId = componentPO.getClusterId();
        String serviceName = componentPO.getServiceName();
        String componentName = componentPO.getName();
        if (clusterId == null || serviceName == null || componentName == null) {
            return;
        }
        Map<String, Object> row =
                alephysStore.latestProcessForComponent(clusterId, serviceName, componentName);
        if (row.isEmpty()) {
            return;
        }
        ...
        if (processId != null) {
            componentVO.setProcessId(String.valueOf(processId));
        }
        if (generation != null) {
            componentVO.setProcessGeneration(Long.parseLong(String.valueOf(generation)));
        }
        if (processDir != null) {
            componentVO.setProcessDir(String.valueOf(processDir));
        }
    }
