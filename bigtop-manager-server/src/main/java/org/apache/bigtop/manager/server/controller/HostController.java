...
public class HostController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<HostVO> get(@PathVariable("id") Long id) {
        return ResponseEntity.success(hostService.get(id));
    }

    @Operation(summary = "update", description = "Update a host")
    @PutMapping("/{id}")
    public ResponseEntity<HostVO> update(@PathVariable("id") Long id, @RequestBody @Validated HostReq hostReq) {
        HostDTO hostDTO = HostConverter.INSTANCE.fromReq2DTO(hostReq);
    ...
    public ResponseEntity<List<ComponentVO>> components(@PathVariable("id") Long id) {
