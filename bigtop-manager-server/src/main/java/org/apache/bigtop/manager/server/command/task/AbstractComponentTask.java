...
public abstract class AbstractComponentTask extends AbstractTask {
    ...
    protected ComponentCommandRequest getComponentCommandRequest() {
        ...
        // CM-style: each configure/start gets a new process dir id; old dirs remain on the agent
        String commandCode = getCommand().getCode();
        if ("configure".equalsIgnoreCase(commandCode)
                || "start".equalsIgnoreCase(commandCode)
                || "restart".equalsIgnoreCase(commandCode)) {
            String processId = System.currentTimeMillis() / 1000
                    + "-"
                    + taskContext.getServiceName()
                    + "-"
                    + taskContext.getComponentName();
            payload.setProcessId(processId);
            recordProcessForUi(processId);
        }
    ...
    private void recordProcessForUi(String processId) {
        try {
            AlephysStore store = SpringContextHolder.getBean(AlephysStore.class);
            ServiceDao serviceDao = SpringContextHolder.getBean(ServiceDao.class);
            ServicePO servicePO =
                    serviceDao.findByClusterIdAndName(taskContext.getClusterId(), taskContext.getServiceName());
            if (servicePO == null) {
                return;
            }
            store.recordProcess(
                    taskContext.getClusterId(),
                    servicePO.getId(),
                    taskContext.getServiceName(),
                    taskContext.getComponentName(),
                    processId);
        } catch (Exception ignored) {
            // UI URL enrichment is best-effort; agent configure must still proceed
        }
    }
