...
TAR_FILE="${DIR_PREFIX}/beat-agent.tar.gz"
TARGET_DIR="${DIR_PREFIX}/beat-agent"
DOWNLOAD_URL="${REPO_URL}/beat-agent.tar.gz"
...
handle_tar_file() {
    ...
    if [ -f "$TAR_FILE" ]; then
        if [ ! -s "$TAR_FILE" ]; then
            info "Removing empty file: $TAR_FILE"
            rm -f "$TAR_FILE"
        elif [ -n "$CHECK_ALG" ]; then
    ...
    if command -v wget &> /dev/null; then
        wget -q --no-check-certificate "$DOWNLOAD_URL" -O "$TAR_FILE"
    else
        curl -skL "$DOWNLOAD_URL" -o "$TAR_FILE"
    fi
