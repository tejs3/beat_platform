...
PID_FILE="${BIGTOP_MANAGER_HOME}/beat-server.pid"
...
is_running() {
    ...
    if [ -n "$running_pid" ]; then
        ...
        echo "Found running BEAT Platform Server process (PID: $running_pid), created PID file"
        return 0
    fi
...
start_server() {
    if is_running; then
        echo "BEAT Platform Server is already running (PID: $(cat $PID_FILE))"
        exit 1
    fi

    echo "Starting BEAT Platform Server..."
    ...
    echo $! > "$PID_FILE"
    echo "BEAT Platform Server started successfully (PID: $!)"
}

stop_server() {
    if ! is_running; then
        echo "BEAT Platform Server is not running"
        return 0
    fi

    local pid=$(cat "$PID_FILE")
    echo "Stopping BEAT Platform Server (PID: $pid)..."
    ...
    rm -f "$PID_FILE"
    echo "BEAT Platform Server stopped successfully"
}
