...
public class ClusterController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ClusterVO> get(@PathVariable("id") Long id) {
        return ResponseEntity.success(clusterService.get(id));
    }

    @Operation(summary = "update", description = "Update a cluster")
    @PutMapping("/{id}")
    public ResponseEntity<ClusterVO> update(@PathVariable("id") Long id, @RequestBody @Validated ClusterReq clusterReq) {
