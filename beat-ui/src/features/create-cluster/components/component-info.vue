...
<script setup lang="ts">
  ...
  const stackSelected = ref('beat')
  const setSourceRef = ref<InstanceType<typeof SetSource>>()
  const stackGroup = shallowRef(['beat', 'infra', 'extra'])
  const stackLabel = (name: string) => {
    const map: Record<string, string> = { beat: 'BEAT', bigtop: 'BEAT', infra: 'Infra', extra: 'Extra' }
    ...
    <a-table :loading="loading" :data-source="data" :columns="columns" :pagination="paginationProps" @change="onChange">
      <template #bodyCell="{ column }">
        <template v-if="column.key === 'stack'">
          <span> {{ `${stackLabel(stackSelected)}-${currentStack?.stackVersion}` }} </span>
        </template>
      </template>
    </a-table>
