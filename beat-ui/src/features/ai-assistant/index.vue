...
<script setup lang="ts">
  ...
  const goAdvisorySuggestions = () => {
    controlVisible(false)
    menuStore.onHeaderClick('/cluster-manage/ai-advisory/suggestions')
  }
    ...
    >
      ...
      <div class="chat">
        <a-spin :spinning="loadingChatRecords">
          <empty-content v-if="noChat" @go-set-up="goSetUpLlmConfig" @go-advisory="goAdvisorySuggestions" />
          <template v-else>
      ...
      <template #footer>
        <chat-input v-if="hasActivePlatform" />
        <div class="ai-assistant-footer-links">
          <a-typography-link @click="goAdvisorySuggestions">
            {{ t('advisory.open_page') }}
          </a-typography-link>
        </div>
        <a-typography-text v-if="hasActivePlatform" type="secondary" class="ai-assistant-desc">
