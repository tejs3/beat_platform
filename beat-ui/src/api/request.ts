  ...
  async (res: AxiosResponse) => {
    if (res.config.responseType === 'stream') {
      ...
      if (responseEntity.code !== 0) {
        const onLoginPage = router.currentRoute.value.path === '/login' || router.currentRoute.value.name === 'Login'
        if (responseEntity.code === 10000 && !onLoginPage) {
          await router.push('/login')
        } else if (responseEntity.code !== 10000 || !onLoginPage) {
