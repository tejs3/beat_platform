...
export const useMenuStore = defineStore(
  ...
  () => {
    ...
    const setupHeader = () => {
      ...
      if (!matchedPath || matchedPath === '/login') {
        headerSelectedKey.value = '/cluster-manage'
      } else {
        headerSelectedKey.value = matchedPath
      }
    ...
    /**
     * Rebuild header/sider from live routes (do not persist trees — UI deploys would hide new items).
     */
    const refreshMenus = () => {
      buildMenuMap()
      setupHeader()
  ...
  },
  {
    persist: {
      ...
      paths: ['siderMenuSelectedKey', 'headerSelectedKey', 'routePathFromClusters']
    }
  }
)
