...
export const useCreateServiceStore = defineStore(
  ...
  () => {
    ...
    const infraServices = computed(() => stackStore.getServicesByExclude(['beat', 'bigtop', 'extra']) as ExpandServiceVO[])
    ...
    function setComponentHosts(compName: string, hosts: HostVO[]) {
      ...
      if (compName.includes('/')) {
        ;[serviceName, componentName] = compName.split('/')
      } else {
        componentName = compName
        serviceName = selectedServices.value.find((svc) =>
          svc.components?.some((comp) => comp.name === componentName)
        )?.name
      }
    ...
    async function attachComponentToService() {
      try {
        ...
        // Only new host assignments — re-sending existing hosts re-runs install/start stages
        const componentCommands: ComponentCommandReq[] = []
        for (const [compName, comp] of allComps.value) {
          const snapHosts = new Set((componentSnapshot.value.get(compName)?.hosts ?? []).map((h) => h.hostname))
          const newHostnames = (comp.hosts ?? [])
            .map((h) => h.hostname!)
            .filter((hostname) => hostname && !snapHosts.has(hostname))
          if (newHostnames.length > 0) {
            componentCommands.push({ componentName: compName!, hostnames: newHostnames })
          }
        }
        if (componentCommands.length === 0) {
          return false
        }
        commandRequest.value.componentCommands = componentCommands
        createdPayload.value = await execCommand({ ...commandRequest.value, clusterId: clusterId.value })
        Object.assign(createdPayload.value, { clusterId: clusterId.value })
        return true
      } catch (error) {
