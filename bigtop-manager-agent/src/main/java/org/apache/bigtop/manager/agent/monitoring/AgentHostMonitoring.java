...
public class AgentHostMonitoring {
    ...
    public static final String NETWORK_RX = "networkRx";
    public static final String NETWORK_TX = "networkTx";
    public static long[] previousReadBytes;
    public static long[] previousWriteBytes;
    public static long previousNetworkRx = -1L;
    public static long previousNetworkTx = -1L;
    ...
    public static Map<ArrayList<String>, Map<ArrayList<String>, Double>> getNetworkIOGauge(JsonNode agentMonitoring) {
        SystemInfo si = new SystemInfo();
        BaseAgentGauge gaugeBaseInfo = new BaseAgentGauge(agentMonitoring);
        ArrayList<String> labels = gaugeBaseInfo.getLabels();
        ArrayList<String> labelBase = gaugeBaseInfo.getLabelsValues();
        labels.add("networkIO");
        Map<ArrayList<String>, Double> labelValues = new HashMap<>();
        ...
        for (NetworkIF nif : si.getHardware().getNetworkIFs()) {
            ...
            rx += Math.max(0L, nif.getBytesRecv());
            tx += Math.max(0L, nif.getBytesSent());
        }
        double rxBps = 0.0;
        double txBps = 0.0;
        if (previousNetworkRx >= 0 && previousNetworkTx >= 0) {
            // collector runs ~every 30s
            rxBps = Math.max(0.0, (rx - previousNetworkRx) / 30.0);
            txBps = Math.max(0.0, (tx - previousNetworkTx) / 30.0);
        }
        previousNetworkRx = rx;
        previousNetworkTx = tx;

        ArrayList<String> rxValues = new ArrayList<>(labelBase);
        rxValues.add(NETWORK_RX);
        labelValues.put(rxValues, rxBps);
        ArrayList<String> txValues = new ArrayList<>(labelBase);
        txValues.add(NETWORK_TX);
        labelValues.put(txValues, txBps);

        Map<ArrayList<String>, Map<ArrayList<String>, Double>> gauge = new HashMap<>();
        gauge.put(labels, labelValues);
        return gauge;
    }
    ...
    public static MultiGauge newNetworkIOMultiGauge(MeterRegistry registry) {
        return MultiGauge.builder("agent_host_monitoring")
                .description("BEAT Agent Host Monitoring, NetworkIO Monitoring")
                .baseUnit("networkIO")
                .register(registry);
    }
    ...
    public static void networkIOMultiGaugeUpdateData(MultiGauge networkIOMultiGauge) {
        try {
            Map<ArrayList<String>, Map<ArrayList<String>, Double>> networkGauge = getNetworkIOGauge(getHostInfo());
            multiGaugeUpdateData(networkIOMultiGauge, networkGauge);
        } catch (UnknownHostException e) {
            throw new RuntimeException("Get agent host monitoring info failed");
        }
    }
