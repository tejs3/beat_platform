...
PID_FILE="${BIGTOP_MANAGER_HOME}/beat-agent.pid"
...
is_running() {
    ...
    if [ -n "$running_pid" ]; then
        ...
        echo "Found running BEAT Platform Agent process (PID: $running_pid), created PID file"
        return 0
    fi
...
start_agent() {
    if is_running; then
        error "BEAT Platform Agent is already running (PID: $(cat $PID_FILE))"
        exit 1
    fi
    ...
    echo "Starting BEAT Platform Agent..."
    ...
    echo $! > "$PID_FILE"
    echo "BEAT Platform Agent started successfully (PID: $!)"
}

stop_agent() {
    if ! is_running; then
        echo "BEAT Platform Agent is not running"
        return 0
    fi

    local pid=$(cat "$PID_FILE")
    echo "Stopping BEAT Platform Agent (PID: $pid)..."
    ...
    rm -f "$PID_FILE"
    echo "BEAT Platform Agent stopped successfully"
}
