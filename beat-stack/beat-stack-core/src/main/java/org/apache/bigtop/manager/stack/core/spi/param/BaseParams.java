...
public abstract class BaseParams implements Params {
    ...
    public ComponentCommandPayload getPayload() {
        return payload;
    }

    /**
     * Package-installed conf path (non-versioned). Process dirs mirror into this
     * so daemons that ignore BEAT process dirs still see the latest configs.
     */
    ...
    /**
     * When {@link ComponentCommandPayload#getProcessId()} is set, return the CM-style
     * process directory; otherwise the legacy package conf path.
     */
    protected String resolveConfDir(String legacyPath) {
        if (payload != null && payload.getProcessId() != null && !payload.getProcessId().isBlank()) {
            return org.apache.bigtop.manager.stack.core.utils.BeatProcessDirs.dirFor(payload.getProcessId());
        }
        return legacyPath;
    }
    ...
    /**
     * service conf dir — CM-style process dir when processId is set
     */
    @Override
