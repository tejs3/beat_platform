...
public class HdfsNameNodeScript extends AbstractServerScript {
    ...
    public ShellResult configure(Params params) {
        ...
        return HadoopSetup.configure(params, "namenode");
    }
    ...
    public ShellResult start(Params params) {
        ...
        try {
            if (namenodeList != null && !namenodeList.isEmpty() && hostname.equals(namenodeList.get(0))) {
                ...
                return startResult;
            } else if (namenodeList != null && namenodeList.size() == 1) {
                HadoopSetup.formatNameNode(hadoopParams);
                String startCmd = MessageFormat.format("{0}/hdfs --daemon start namenode", hadoopParams.binDir());
                return LinuxOSUtils.sudoExecCmd(startCmd, hadoopParams.user());
            } else {
                throw new StackException("Current host is not in NameNode list: " + hostname);
            }
        } catch (Exception e) {
