...
import org.apache.bigtop.manager.stack.core.utils.BeatProcessDirs;
    ...
    public static ShellResult execute(ComponentCommandPayload payload) {
        try {
            ...
            if (Environments.isDevMode()) {
                ...
                boolean processLifecycle = "configure".equalsIgnoreCase(command)
                        || "start".equalsIgnoreCase(command)
                        || "restart".equalsIgnoreCase(command);
                if (processLifecycle) {
                    BeatProcessDirs.prepare(params);
                }
                ...
                } else if (processLifecycle) {
                    BeatProcessDirs.finalizeProcess(params);
                }
